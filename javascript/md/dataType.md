...
`null`은 의도적으로 **기본형(primitives) 또는 object형 변수에 값이 없음** 을 명시한 것이다.

설계상의 문제로 `typeof` 연산자(데이터 형식을 나타내는 문자열을 반환한다)로 null값을 가진 변수를 연산하면 null이 아닌 object가 나온다.  
->> null 타입 변수인지 확인할 때는 typeof 연산자가 아닌 일치 연산자(===)를 사용해야 한다.
```javascript
var foo = 'chi';
...
자바스크립트는 하나의 숫자 자료형(배정밀도 64비트 부동 소수점형)만 존재한다.  
정수만 표현하기 위한 특별한 자료형<sup>integer type</sup>은 없다.

> 배정밀도 64비트 부동 소수점형<sup>[double-precision 64-bit floating-point](https://en.wikipedia.org/wiki/Double-precision_floating-point_format)</sup> : -(2<sup>53</sup> -1) 와 2<sup>53</sup> -1 사이의 숫자값
...
C와 같은 언어와는 다르게, 자바스크립트의 문자열은 변경 불가능<sup>immutable</sup>하다.  
...
이 문자열은 Symbol 생성에 어떠한 영향도 주지 않지만  
생성된 Symbol에대한 설명<sup>description</sup>으로 디버깅 용도로만 사용된다.
...
**객체 프로퍼티 키로 사용해서 얘기치 않게 프로퍼티 키와 충돌하는 일을 방지한다.** 
```javascript
const obj = {};
...
Symbol() 함수를 통해 Symbol을 생성할 수 있다는 것은 **Symbol이 함수 객체** 라는 의미이다.  
...
아래는 `Symbol.iterator`를 key로 사용한 메서드를 구현하고 있는 빌트인 객체(빌트인 이터러블)이다.  
...
/* 
 ...
 value, done  프로퍼티를 갖는 객체를 반환하는 next() 함수를 메서드로 같는 객체이다. 이터레이터의 next()  메서드를 통해 이터러블 객체를 순회할 수 있다.
*/
